...
static void generate_coinbase(ckpool_t *ckp, workbase_t *wb)
{
	...
	memcpy(wb->coinb2bin, "\x0a\x62\x69\x74\x63\x6f\x69\x6e\x63\x61\x73\x68\x70\x6f\x6f\x6c\x2e\x6f\x72\x67", 20);
	wb->coinb2len = 20;
	if (ckp->btcsig) {
			    ...
			    const json_t *params_val, json_t **err_val)
{
	...
	bool block_reset_happened = (client->best_diff == 0 && sdiff >= wb->network_diff);
	if (!block_reset_happened && sdiff > client->best_diff) {
		worker_instance_t *worker = client->worker_instance;
		...
		else if (block_reset_happened) {
		/* Если это блок, мы пропустили код выше, чтобы не портить best_diff нового раунда.
		   Но мы ОБЯЗАНЫ вручную обновить best_ever, так как это рекорд. */
		worker_instance_t *worker = client->worker_instance;
